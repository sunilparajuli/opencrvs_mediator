...
class WebhookEventView2(APIView):
    """
    ...
    def post(self, request):
        ...
        # Identify CHILD, FATHER, and MOTHER
        for patient_resource in patient_resources:
            # Check for CHILD using BIRTH_REGISTRATION_NUMBER
            if any(
            ...
            # Check for FATHER based on gender
            import pdb;pdb.set_trace(s)
            if patient_resource.get("gender") == "male" and not father:
                father = patient_resource
                logger.info("Identified FATHER patient.")
                continue

            # Check for MOTHER based on gender
            if patient_resource.get("gender") == "female" and not mother:
                mother = patient_resource
...
class WebhookEventView3(APIView):
    """
    Handles incoming webhook events and processes Patient and Person resources.
    """
    def post(self, request):
        ...
        for patient_resource in patient_resources:
            gender = patient_resource.get("gender", "").lower().strip()
        ...
        # Identify FATHER and MOTHER from the remaining resources
        for patient_resource in patient_resources:
            if not father and patient_resource.get("gender") == "male":
                father = patient_resource
                logger.info("Identified FATHER patient.")
                continue

            if not mother and patient_resource.get("gender") == "female":
                mother = patient_resource
                logger.info("Identified MOTHER patient.")
                continue
